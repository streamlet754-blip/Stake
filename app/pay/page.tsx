...
import { PaymentForm, CouponForm } from "@/components/payment-form";
...
export default async function PayPage() {
  try {
      ...
      return <main className="shell payment-shell"><nav className="nav"><Link className="logo" href="/">bli<span>t</span>le</Link><Link className="nav-link" href="/">Back home <span aria-hidden="true">↗</span></Link></nav><section className="payment-page"><div className="payment-intro"><div className="kicker">Blitle Premium <span className="kicker-line" /></div><h1>Unlock your <em>watch.</em></h1><p className="lede">One simple license for your Garmin installation. Pay directly, then verify the transaction below.</p><div className="trust-row"><span>Ethereum mainnet</span><span>2.34 USDT</span><span>One device</span></div></div><div className="checkout-steps"><span className="step-active"><b>01</b> Send payment</span><i /><span><b>02</b> Verify ownership</span></div><div className="payment-grid"><section className="panel payment-card"><div className="card-heading"><div><div className="eyebrow">Step 01</div><h2>Send payment</h2></div><div className="amount">2.34 <small>USDT</small></div></div><div className="qr-wrap"><img className="qr" src={qr} alt="Public payment address QR code" /></div><div className="address-label"><span>Destination address</span><span>Ethereum mainnet</span></div><div className="address-row"><strong>{config.recipientAddress}</strong><CopyAddress address={config.recipientAddress} /></div><div className="notice"><b>Send exactly USDT on Ethereum mainnet.</b> Other tokens and networks may not be recognized.</div></section><section className="panel verify-card"><div className="eyebrow">Step 02</div><h2>Verify payment</h2><p>Paste your transaction hash after the transfer is confirmed.</p><PaymentForm /><CouponForm /></section></div></section></main>;
    } catch {
