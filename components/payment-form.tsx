...
export function PaymentForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const response = await fetch("/api/verify-payment", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ transactionHash: hash, couponCode: couponCode || undefined }) });
  ...
  return <form onSubmit={submit}><label className="label" htmlFor="hash">Transaction hash</label><div className="row"><input className="input" id="hash" value={hash} onChange={(event) => setHash(event.target.value)} placeholder="0x..." required /><button className="button" type="submit">Verify Payment</button></div><label className="label coupon-label" htmlFor="coupon-code">Discount code <span>Optional</span></label><input className="input" id="coupon-code" value={couponCode} onChange={(event) => setCouponCode(event.target.value)} placeholder="LIC30 or GOF10" autoCapitalize="characters" />{message && <div className="notice" role="status">{message}</div>}{license && <div className="panel"><div className="kicker">Your Blitle license</div><h2>{license}</h2><p>Save this key. Enter it in the Garmin app to activate premium functionality.</p></div>}</form>;
}
